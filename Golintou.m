...
% (como 'clear' e 'clc') para não afetar o ambiente do usuário.
    ...
    function [] = executarCalculos(~, ~)
        ...
        for k = 1:numFields
            ...
            if strcmp(fname, 'arquivo')
                ...
                % tentar converter para número (escalares)
                numVal = str2double(valText);
                if ~isnan(numVal)
                    data.(fname) = numVal;
                else
                    % fallback: manter como texto se não converter
                    data.(fname) = valText;
                end
            end
        end
        ...
        if fid == -1
            warning('Não foi possível abrir o arquivo "%s": %s', arquivo, emsg);
            return;
        end
        file = textscan(fid,'%f%f%f%f');
        fclose(fid);
        if isempty(file) || any(cellfun(@isempty,file))
            warning('Conteúdo inesperado em "%s". Esperadas 4 colunas numéricas.', arquivo);
            return;
        end
        ...
        flagni = 1;
        counter = 0;
        % prealoca vetores (tamanho máximo igual ao nº de segmentos)
        nseg = length(xi);
        ...
        % prealoca pontos de interseção (limite superior N*(N+1)/2)
        ...
        if flagin==1 & flagni==1
            ...
            % garantir M7p válido mesmo quando M7 estiver vazio
            M7p = zeros(0,4);
            counter = 0;
            for n=1:2:length(M7)
                counter = counter+1;
